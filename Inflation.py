...
def VGrid(Cash, Time, StartRate, EndRate, SepRate):
    ...
    Rate = RateFunc(StartRate, EndRate, SepRate) #Uses functions from earlier
    Time = TimeFunc(Time)

    Timek, Ratek = np.meshgrid(Time, Rate, indexing="ij") #Function Broadcasting using meshgrid
    
    VGrid = Vdecay(Ratek, Timek, Cash) #VGrid for all combinations

    Imaging = plt.contourf(Timek, Ratek, VGrid) #Images and shows basic contours of the inflation rate scenarios
    plt.colorbar()
    plt.show()
    
    return VGrid

VGrid(1000, 50, 0, 5, 0.1) #Can delete later just placeholder values
